...
import cc.makepower.cc_door_face.bean.FaceRegisterInfo;
import cc.makepower.cc_door_face.utils.ImageUtil;
...
public class FaceServer {
    ...
    public static final String SAVE_IMG_DIR = "makepower_f" + File.separator + "imgs";
    public static final String SAVE_FEATURE_DIR = "makepower_f" + File.separator + "features";
    ...
    public boolean init(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ROOT_PATH = context.getFilesDir().getAbsolutePath();//正式环境
...
//                return;
    ...
    public void delectTempFace() {
        synchronized (this) {
//            File featureDir = new File(Environment.getExternalStorageDirectory().getPath() + File.separator + "TempFaces" + File.separator + SAVE_FEATURE_DIR);
            deleteDir(Environment.getExternalStorageDirectory().getPath() + File.separator + "TempFaces");
        }
    }
    ...
    public String registerTempFace(Context context, byte[] nv21, int width, int height, String name) {
        synchronized (this) {
            ...
            //图片存储的文件夹
            File imgDir = new File(Environment.getExternalStorageDirectory().getPath() + File.separator + "TempFaces" + File.separator + SAVE_IMG_DIR);
            if (!imgDir.exists()) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        return imgDir + File.separator + userName + IMG_SUFFIX + "&" + featureDir + File.separator + userName;
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
