...
public class DrawHelper {
    ...
    public void transFormationDraw(FaceRectView faceRectView, List<DrawInfo> drawInfoList) {
...
//        }
    ...
    public static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness) {
        ...
        if (!TextUtils.isEmpty(drawInfo.getName())) {
            paint.setTextSize(rect.width() / 8);
//            String str = "(" + (drawInfo.getAge() == AgeInfo.UNKNOWN_AGE ? "未知年龄" : drawInfo.getAge() + "岁") + ")";
            canvas.drawText(drawInfo.getName() , rect.left, rect.top - 10, paint);
//            String str = (drawInfo.getSex() == GenderInfo.MALE ? "男性" : (drawInfo.getSex() == GenderInfo.FEMALE ? "男性" : "未知"))
//                    + ","
//                    + (drawInfo.getAge() == AgeInfo.UNKNOWN_AGE ? "未知" : drawInfo.getAge())
//                    + ","
//                    + (drawInfo.getLiveness() == LivenessInfo.ALIVE ? "有活性" : (drawInfo.getLiveness() == LivenessInfo.NOT_ALIVE ? "无活性" : "未知"));
